-- Fix: Column reference "linked_at" is ambiguous in accept_parent_student_invitation
-- The RETURNS TABLE column names conflict with aliases in the SELECT statement
-- Solution: Remove aliases since RETURNS TABLE already defines the column names
...
BEGIN
  ...
  -- Don't use aliases - RETURNS TABLE column names are used automatically
  RETURN QUERY
  SELECT
    v_link_id,
    v_parent_id,
